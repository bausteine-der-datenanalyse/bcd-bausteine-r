...
```{r}
ggplot(data = d_donau) +
  geom_histogram(mapping = aes(x = Abfluss), binwidth = 30) +
...
```{r}
ggplot(data = d_donau) +
  stat_ecdf(mapping = aes(x = Abfluss)) +
...
ggplot(data = d_donau) +
  geom_histogram(
    mapping = aes(x = Abfluss, y = after_stat(density)),
    binwidth = 30
  ) +
  stat_function(
    fun = \(x) dnorm(x, mean = mean(d_donau$Abfluss), sd = sd(d_donau$Abfluss)),
    color = "red"
  )
```
