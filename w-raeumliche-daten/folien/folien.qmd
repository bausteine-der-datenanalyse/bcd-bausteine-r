...
![GIS Layer, Quelle: http//landportal.org](bilder/GISLayers.png)
...
- Weltkarte mit `ne_countries()` als `sf`-Objekt ([weitere Infos hier](http://www.naturalearthdata.com))
...
![](bilder/simple-feature-area-2.png)
...
- Dataframe mit `wb_data()` (Paket `wbstats`) herunterladen
  - Indikator [hier](https://data.worldbank.org) heraussuchen
...
# Werte zu einzelnen Koordinaten
...
## Landesgrenzen Bundesrepublik 1/2 {.smaller}
...
- Datensatz laden mit `gisco_get_nuts`
  - `country` Land
  - `nuts_level` gibt Ebene an [Staat/Bundesländer/Regierungsbezirke](https://de.wikipedia.org/wiki/NUTS:DE)
  - `resolution` Auflösung, siehe Dokumentation
  - [NUTS](https://de.wikipedia.org/wiki/NUTS) ist ein System zur Bezeichnung von Ländern

## Landesgrenzen Bundesrepublik 2/2 {.smaller}
...
- Objekte zu Anfrage hinzufügen mit `add_osm_feature`
  ...
  - [Erläuterung hier](https://wiki.openstreetmap.org/wiki/Map_features)
...
```{r}
#| echo: false
# Heruntergeladene Daten einlesen - Download nicht zuverlässig
load(file = "daten/strassen-bochum-download.RData")
s1 <- s1a
```
...
- [Hervorragendes Tutorial](https://jcoliver.github.io/learn-r/017-open-street-map.html)
