...
library(tidyverse)
library(kableExtra)
load("daten/einzelne-merkmale-lecture.Rdata")
```
...
## Treibhausgasemissionen (2012) bunt
...
- Für die x-Position werden jetzt die Jahre verwendet

## Anteilige Treibhausgasemissionen
...
-   Mit `position="fill"` erhalten alle Balken erhalten dieselbe Höhe
-   Damit werden Anteile ablesbar

## Pro-Kopf-Emissionen über die Zeit
...
-   Der x-Wert fest vorgegeben
-   Höhe entsprechend Merkmal B, Füllfarbe nach Merkmal A
...
-   Die y-Koordinate wird für den Winkel verwendet
...
-   Mit einer statistischen Transformation

## Statistische Transformation

![Statistische Transformation](bilder/geom_bar_transformation.svg)
...
-   Im Jahr 2012 kommt jedes Land nur einmal vor
-   Regionen zählen ergibt Anzahl der Länder pro Region

## Anzahl der Länder pro Region bunt
...
-   Balken entsprechend der Region gefärbt (Information doppelt)
...
-   Vertikale Position und Farbe **nach** den Aesthetics angeben (vorgegebene Eigenschaften)
...
-   Voreinstellung nicht gut (siehe Info-Meldung)
...
-   Welche Grenze angegeben wird spielt keine Rolle
...
-   Angeben welche Seite geschlossen sein soll
...
-   Mit `y = stat(density)` wird die relative Häufigkeit berechnet
...
-   Klassen mit `breaks = c(c0, c1, ..., ck)` angeben
-   Fläche der Rechtecke ergibt in der Summe 1
...
-   Damit: Empirische Verteilungsfunktion für Merkmal X
