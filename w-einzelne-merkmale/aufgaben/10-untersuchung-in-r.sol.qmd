...
ggplot(data = d_bo_entfernung) +
  geom_col(mapping = aes(x = Stadt, y = Entfernung))
```
...
ggplot(data = d_bo_vm) +
  geom_bar(
    mapping = aes(x = Fachbereich, fill = Verkehrsmittel),
    position = "fill"
  )
```
...
ggplot(data = d_bo_vm) +
  geom_histogram(
    mapping = aes(x = Zeit, fill = Verkehrsmittel),
    boundary = 0,
    binwidth = 15
  )
```
