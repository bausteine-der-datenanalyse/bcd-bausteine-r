...
Dieser Text ist eine kurze Einführung in das Schreiben mit [Quarto](www.quarto.org). Quarto ist ein Programmsystem zur Erstellung wissenschaftlicher und technischer Dokumente. Sie können damit
...
Ein Tutorial zur Verwendung von Quarto in RStudio finden Sie unter <https://quarto.org/docs/get-started/hello/rstudio.html>.
...
Als Ausgabeformat wird `typst` verwendet um ein PDF-Dokument zu erzeugen. Dabei steht `toc: true` dafür, dass ein Inhaltsverzeichnis eingefügt wird (toc = *Table of Contents*), `number-sections: true` für nummerierte Abschnitte und `section-numbering: 1.1.1` für das Nummerierungschema. Abschließend wird noch angegeben, dass der R-Code nicht mit ausgegeben wird.
...
-   Zur Verwaltung der Literaturliste ist das Programm [Zotero](https://www.zotero.org) sehr gut geeignet. Es lassen sich damit Literaturstellen sammeln und im BibTeX-Format exportieren. Im visuellen Editor von RStudio lassen sich Zotero-Referenzen sehr komfortabel einfügen.

-   Das Aussehen der PDF-Dokumente kann angepasst werden, [hier ein kurzes Video zur Einführung](https://youtu.be/2DbuqYKOsrY?si=oVfzelhrEbnCLPJv&t=715).

-   Eine etwas tiefergehende Einführung in das wissenschaftliche Schreiben mit Quarto finden Sie [hier](https://quarto.org/docs/visual-editor/technical.html).
