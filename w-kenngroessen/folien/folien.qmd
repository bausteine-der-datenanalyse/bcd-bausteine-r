---
title: "Kenngrößen von Verteilungen"
---
...
- Merkmal `X` von Dataframe `d` als Array mit `d$X`
...
## Beobachtungszeitraum ist Spannweite von Merkmahl `Jahr`

```{r}
max(d_ns_m$Jahr) - min(d_ns_m$Jahr)
```

## Mittelwerte, Mediane und Standardabweichungen der Niederschläge
...
(Die Zahlenwerte aus dem Skript)
...
- Ergebnis ist ein _Named Vector_
...
## Erläuterung
- Mit `factor(Jahr)` teilen wir explizit mit, dass `Jahr` ein diskretes Merkmal ist
...
- Normierung auf $G^*$ von Hand falls notwendig (mit Faktor n/(n-1) multiplizieren)
