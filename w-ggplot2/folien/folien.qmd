...
- Beispiele unter https://www.r-graph-gallery.com

→ Und warum heißt das Paket jetzt ggplot?
...
### Dataframes `d_wb_all` und `d_wb_2012` aus Datei laden
...
| Name      | Inhalt                                    |
|-----------|-------------------------------------------|
| `d_wb_all`  | Weltbank - Alle Daten (vorherige Folie)   |
...
```{r}
ggplot(data = d_bsp) +
  geom_point(mapping = aes(x = A, y = B, color = C, size = D)) + coord_fixed()
```
